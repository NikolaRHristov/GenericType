...
export function createReactPlugin(options = {}) {
	...
	const plugin: Plugin = {
		...
		afterCreate: async (reactive: Reactive<T>): Promise<void> => {
		...
		beforeUpdate: async (newValue: T, oldValue: T): Promise<T> => {
			// Handle batched updates for React components
			const updates = Array.from(components.entries()).filter(
				([id, component]) => component.instances.size > 0,
			);
		...
		afterUpdate: async (value: T): Promise<void> => {
		...
		beforeDestroy: async (reactive: Reactive<T>): Promise<void> => {
			const component = components.get(reactive.id);
			if (component) {
		...
		handleError: async (
			error: Error,
			context: { phase: string; config: ReactiveConfig<any> },
		) => {
	...
	function registerInstance(componentId: string, instanceId: symbol) {
		...
		if (component) {
			...
			const reactive = systemContext.getReactive(componentId);
			if (reactive) {
				reactive.meta.custom.react.instanceCount =
...
function ExampleComponent() {
	...
	const { value, setValue, meta } = system.hookFactory.useReactive(
		componentId,
		{
			immediate: true,
			onError: (error) => {
